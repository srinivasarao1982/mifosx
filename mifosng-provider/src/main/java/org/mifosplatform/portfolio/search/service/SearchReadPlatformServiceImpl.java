    ...
    private static final class SearchMapper implements RowMapper<SearchData> {
        ...
        public String searchSchema(final SearchConditions searchConditions) {
            ...
            final String loanExactMatchSql = " (select 'LOAN' as entityType, l.id as entityId, pl.name as entityName, l.external_id as entityExternalId, l.account_no as entityAccountNo "
                    ...
                    + " from m_loan l left join n_loan_ext ext on ext.loan_id = l.id join m_client c on l.client_id = c.id join m_office o on o.id = c.office_id join m_product_loan pl on pl.id=l.product_id where o.hierarchy like :hierarchy and (l.account_no like :search or l.external_id like :search or ext.loanApplication_Id like :search )) ";
            ...
            final String loanMatchSql = " (select 'LOAN' as entityType, l.id as entityId, pl.name as entityName, l.external_id as entityExternalId, l.account_no as entityAccountNo "
                    + " , c.id as parentId, c.display_name as parentName, null as entityMobileNo, l.loan_status_id as entityStatusEnum ,null as entityExternalId2,ext.loanApplication_Id as loanApplicationId "
                    + " from m_loan l left join n_loan_ext ext on ext.loan_id = l.id join m_client c on l.client_id = c.id join m_office o on o.id = c.office_id join m_product_loan pl on pl.id=l.product_id where o.hierarchy like :hierarchy and "
                    + " ((l.account_no like :partialSearch and l.account_no not like :search) or (l.external_id like :partialSearch and l.external_id not like :search) or (ext.loanApplication_Id like :partialSearch and ext.loanApplication_Id not like :search))) ";
            ...
            final String savingExactMatchSql = " (select 'SAVING' as entityType, s.id as entityId, sp.name as entityName, s.external_id as entityExternalId, s.account_no as entityAccountNo "
                    + " , c.id as parentId, c.display_name as parentName, null as entityMobileNo, s.status_enum as entityStatusEnum,null as entityExternalId2 ,null as loanApplicationId"
            ...
            final String clientIdentifierExactMatchSql = " (select 'CLIENTIDENTIFIER' as entityType, ci.id as entityId, ci.document_key as entityName, "
                    + " null as entityExternalId, null as entityAccountNo, c.id as parentId, c.display_name as parentName,null as entityMobileNo, c.status_enum as entityStatusEnum ,null as entityExternalId2,null as loanApplicationId"
            ...
            final String groupExactMatchSql = " (select IF(g.level_id=1,'CENTER','GROUP') as entityType, g.id as entityId, g.display_name as entityName, g.external_id as entityExternalId, NULL as entityAccountNo "
                    + " , g.office_id as parentId, o.name as parentName, null as entityMobileNo, g.status_enum as entityStatusEnum ,null as entityExternalId2,null as loanApplicationId"
        ...
        public SearchData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final String entityExternalId2=rs.getString("entityExternalId2");
            ...
            return new SearchData(entityId, entityAccountNo, entityExternalId, entityName, entityType, parentId, parentName,
                    entityMobileNo, entityStatus,entityExternalId2,loanApplicationId);
        }
