    ...
    private static final class RblCenterDatMapper implements RowMapper<RblCenterValidateData> {
        ...
        public String schema() {
            return   "  mg.external_id as externalId,mg.display_name as centerName,mg.activation_date,s.display_name as serviceAgent, "
                     +" (select value from c_configuration where name='allowed-cente- max-customers') as maxIndividual,mrblc.meting_time as meetingTime,concat(ifnull(mrblc.house_no,''),ifnull(mrblc.street_no ,'')) as addressline1 ,"
    ...
    private static final class RblClientDatMapper implements RowMapper<RblclientDatValidation> {
        ...
        public RblClientDatMapper() {}
            ...
            return   "   mc.external_id as externalId,center.external_id as externalCenterId, title.code_score as title,mc.display_name as customerName, "
                  ...
                  +"rblbranch.`bc branch code` as bcBranchCode,s.display_name as collector,staff.display_name as approver , "
                 ...
                 +"left join m_staff s on s.id =center.staff_id "
                 +"left join (select staff_id,center_id from m_task  where id in (select max(id) from m_task where task_type in (select id from m_code_value where code_value='PSM GRT') and  task_status=200 group by center_id)) psmStaff on psmStaff.center_id=center.id  "
                 +"left join m_staff staff on staff.id=psmStaff.staff_id "
        ...
        public RblSavingDatMapper() {}
         ...
         public String schema() {
            return   "   msa.external_id as externalId,center.external_id as externalCenterId,mc.external_id as customerExternalId,"
                     ...
                     +"na.pin_code as nomineePincode,na.pin_code as gurdianPincode,s.display_name as collector,staff.display_name as approver "
                     +" from m_client mc "
                     +"left join m_group_client mgc on mgc.client_id =mc.id "
                     +"left join m_group mg on mgc.group_id =mg.id "
                     +"left join m_group center on center.id =mg.parent_id and center.level_id =1 "
